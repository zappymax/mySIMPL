...
eval(funcCall(ID, B), Var_list_Glob, Var_list_Glob_out, Func_list, Func_list_out, SCOPE, Number):-
    ...
    put_assoc(Param, NewScopeList, Val, NewScopeListA),
    put_assoc(FUNCSCOPE, Var_list_Glob, NewScopeListA, Var_list_Glob_temp),
    %declScopes(Var_list_Glob_temp, Param, Val, FUNCSCOPE, Var_list_Glob_out),
    evaluateProg(FuncLogic, Var_list_Glob_temp, Var_list_Glob_new, Func_list, Func_list_out, FUNCSCOPE, Number).
...
t135 :- do_test(['var', 'x', '<-', 1, ';', 'if', '(', '(', 'true', '||', 'false', ')', ')', 'then', 'var', 'y', '<-', 2, '.', 'endif', ';', 'if', '(', 'x', '==', 1, ')', 'then', 'x', '<-', 'y', '.', 'endif', ';', 'return', 'x', '.'],e).
